...
public class NotificationController {
    ...
    /**
     * POST localhost:8080/notification
     * {
     *     "contactId ":2",
     *     "title":"Ogetto dell 'email"'
     *     "Test":"Testo dell'email"
     * }
     */
    ...
    public ResponseEntity sendNotification(@RequestBody NotificationDTO payload){
        try{
           ...
           // emailService.sentMime(studentToNotify.getEmail(), payload.getTitle(), payload.getText());----- non funziona
            emailService.sentMime(studentToNotify.getEmail(), payload.getTitle(), payload.getText());
